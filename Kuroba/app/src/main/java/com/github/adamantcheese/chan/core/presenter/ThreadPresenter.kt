...
class ThreadPresenter @Inject constructor(
  ...
  private fun saveInternal(): Boolean {
    ...
    if (oldPin != null) {
      ...
      check(watchManager.createPin(loadable, op, PinType.DOWNLOAD_NEW_POSTS, false)) { "Could not create pin for loadable $loadable" }
  ...
  private fun startSavingThreadInternal(loadable: Loadable?, postsToSave: List<Post>, newPin: Pin): Boolean {
    check(PinType.hasDownloadFlag(newPin.pinType)) { "newPin does not have DownloadFlag: " + newPin.pinType }
  ...
  val isPinned: Boolean
    get() {
      if (!isBound) return false
      val pin = watchManager.findPinByLoadableId(loadable!!.id) ?: return false
  ...
  override fun onListScrolledToBottom() {
    if (!isBound) return
    if (chanLoader!!.thread != null && loadable!!.isThreadMode && chanLoader!!.thread!!.postsCount > 0) {
  ...
  override fun onPostClicked(post: Post) {
    if (isBound && loadable!!.isCatalogMode) {
      val newLoadable = Loadable.forThread(loadable!!.site, post.board, post.no, PostHelper.getTitle(post, loadable))
      highlightPost(post)
      val threadLoadable = databaseManager.databaseLoadableManager[newLoadable]
  ...
  override fun onThumbnailClicked(postImage: PostImage, thumbnail: ThumbnailView) {
    ...
    for (post in posts) {
      for (image in post.postImages) {
        ...
        val setCallback = (!post.deleted.get() || image.isFromArchive
  ...
  override fun onPostLinkableClicked(post: Post, linkable: PostLinkable) {
    if (linkable.type == PostLinkable.Type.QUOTE && isBound) {
      val linked = findPostById((linkable.value as Int).toLong(), chanLoader!!.thread)
      if (linked != null) {
    ...
    if (linkable.type == PostLinkable.Type.LINK) {
      threadPresenterCallback!!.openLink(linkable.value as String)
      return
    }

    if (linkable.type == PostLinkable.Type.THREAD && isBound) {
      val link = linkable.value as ThreadLink
      val board = loadable!!.site.board(link.board)
      if (board != null) {
        val loadable = Loadable.forThread(board.site, board, link.threadId.toLong(), "")
        val thread = databaseManager.databaseLoadableManager.get(loadable)
    ...
    if (linkable.type == PostLinkable.Type.BOARD && isBound) {
      val board = databaseManager.runTask(databaseManager.databaseBoardManager
        .getBoard(loadable!!.site, linkable.value as String))
      if (board == null) {
        AndroidUtils.showToast(context, R.string.site_uses_dynamic_boards)
      } else {
        val catalog = databaseManager.databaseLoadableManager[Loadable.forCatalog(board)]
    ...
    if (linkable.type == PostLinkable.Type.SEARCH && isBound) {
      val search = linkable.value as SearchLink
      val board = databaseManager.runTask(databaseManager.databaseBoardManager
