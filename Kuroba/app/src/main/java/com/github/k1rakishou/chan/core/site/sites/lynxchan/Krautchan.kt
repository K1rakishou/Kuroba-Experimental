...
class Krautchan : LynxchanSite() {
  ...
  private val siteUrlHandler = lazy { KrautchanUrlHandler(domainUrl.value, mediaHostsLazy.value) }
  ...
  override val endpoints: Lazy<LynxchanEndpoints>
    get() = krautchanEndpoints
  override val postingViaFormData: Boolean
    get() = true

  class KrautchanUrlHandler(baseUrl: HttpUrl, mediaHosts: Array<HttpUrl>) : BaseLynxchanUrlHandler(
    url = baseUrl,
    mediaHosts = mediaHosts,
    names = arrayOf("Krautchan"),
    siteClass = Krautchan::class.java
  )
