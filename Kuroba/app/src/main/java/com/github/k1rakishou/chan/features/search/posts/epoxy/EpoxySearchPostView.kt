package com.github.k1rakishou.chan.features.search.posts.epoxy
...
import com.github.k1rakishou.chan.features.search.posts.data.ThumbnailInfo
...
@ModelView(autoLayout = ModelView.Size.MATCH_WIDTH_WRAP_HEIGHT) class EpoxySearchPostView @JvmOverloads constructor(
  ...
  fun setPostOpInfo(postOpInfo: CharSequence?) {
    if (postOpInfo == null) {
      searchPostOpInfo.text = null
      searchPostOpInfo.setVisibilityFast(GONE)
      return
    }

    searchPostOpInfo.text = postOpInfo
    searchPostOpInfo.setVisibilityFast(VISIBLE)
  ...
  fun setPostInfo(postInfo: CharSequence) {
    if (postInfo.isEmpty()) {
      ...
      searchPostInfo.setVisibilityFast(GONE)
      return
    }

    searchPostInfo.text = postInfo
    searchPostInfo.setVisibilityFast(VISIBLE)
  ...
  fun setPostComment(comment: CharSequence) {
    if (comment.isEmpty()) {
      ...
      searchPostCommentContainer.setVisibilityFast(GONE)
      return
    }

    searchPostComment.text = comment
    searchPostCommentContainer.setVisibilityFast(VISIBLE)
  ...
  fun setThumbnail(thumbnailInfo: ThumbnailInfo?) {
    ...
    if (thumbnailUrl == null) {
      searchPostThumbnail.setImageBitmap(null)
      searchPostImagesContainer.setVisibilityFast(GONE)
      return
    }

    searchPostImagesContainer.setVisibilityFast(VISIBLE)
