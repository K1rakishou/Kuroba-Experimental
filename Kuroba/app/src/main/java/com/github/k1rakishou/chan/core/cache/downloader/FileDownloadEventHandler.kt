    ...
    try {
      ...
      when (fileDownloadEvent) {
        ...
        is FileDownloadEvent.Success -> {
          ...
          val downloadedAsString = ChanPostUtils.getReadableFileSize(downloaded)
          val totalAsString = ChanPostUtils.getReadableFileSize(total)

          Logger.debug(TAG) {
            "Success (" +
              "cacheFileType: ${cacheFileType}, " +
              "downloaded: ${downloadedAsString} (${downloaded}B), " +
              "total: ${totalAsString} (${total}B), " +
